...
### システム設計から実装、Dockerによるコンテナ化、Qittaでの発信まで
...
3. **得られた学び**
      ...
      通信・暗号・並列処理を含むシステム設計と実装や、`Docker`によるコンテナ化の経験
...
<ul>
  <li>
    <h3>GitHub Actions による自動テスト</h3>
    <p>テストコードを追加し、GitHub Actions によって push 時に自動でテストを実行できるようにします。</p>
    <p>CI（継続的インテグレーション）により、コードの品質維持と開発効率の向上を目指します。</p>
  </li>
  <br>
  <li>
