...
**以下は2人のユーザがルームを作って、チャットをするデモ動画です**
...
TCP・UDPソケットやマルチスレッド、独自プロトコルの実装を通じて、ネットワークへの理解を深めるため。
...
### クライアント状態の一元管理
  ...
  トークンをキーとした辞書`clients_map`に、ユーザー名・ルーム名・ホスト判定・最終アクティブ時間を集約。

  定期チェックで整合性を保ち、状態を安全に管理。
