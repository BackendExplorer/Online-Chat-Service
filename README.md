...
### システム設計から実装、Dockerによるコンテナ化とCI自動テスト、Qiitaでの発信まで対応
...
3. **得られた学び**
      ...
      通信・暗号・並列処理を含むシステム設計と実装や、`Docker`によるコンテナ化、Github Actionを用いた自動テストの経験
...
<ul>
  <li>
    ...
    <p>コードの push やプルリクエスト作成時にワークフローが自動で実行され、<br> <code>pytest</code> によるユニットテストが走ります。</p>
