...
flowchart TD
    ...
    %%─── フロー定義 ───
    Start --> 選択
    選択 -- 作成 --> 入力作成 --> ルーム名入力 --> パスワード設定 --> チャット中
    選択 -- 参加 --> 入力参加 --> ルーム一覧 --> パスワード入力 --> チャット中
...
| 暗号技術       | ![PyCryptodome](https://img.shields.io/badge/Encryption-PyCryptodome-blue) <br>`RSA暗号`で通信
...
<ul>
  <li>
    ...
    <p>Electron を用いてチャットのクライアント操作を GUI 化し、直感的なユーザーインターフェースを提供。</p>
    <p>ユーザー名・ルーム名・パスワードの入力、チャット表示、通知などをウィンドウ上で視覚的に操作可能にします。</p>
    <p>Python の既存クライアントと Node.js をブリッジし、Electron 側から通信処理を統合する設計を想定しています。</p>
  </li>
</ul>
