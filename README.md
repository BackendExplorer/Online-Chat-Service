...
<ul>
  <li>
    <h3>⚙️ TCP と UDP の並列処理の設計</h3>
    <p><strong>背景：</strong><br>
      正確な通信が必要な部分（ルーム作成・参加など）には <code>TCPソケット</code> を、<br>
      リアルタイム性が求められるチャット送信には <code>UDP</code> を採用しました。
    </p>
    <p><strong>工夫：</strong><br>
      <code>TCPServer</code> のクラス変数にクライアント・ルーム情報を一元管理し、<br>
      <code>UDPServer</code> 側からもそれを参照できるようにすることで、<br>
      両サーバ間で状態を正しく共有できるようにしました。
    </p>
    <p><strong>効果：</strong><br>
      状態の整合性を維持しながら、並列処理によってスムーズかつ高速なチャット体験を提供できました。
    </p>
  </li>
  ...
  <li>
    ...
    <p><strong>背景：</strong><br>
      一定時間操作がないユーザーが残ると、リソースの無駄遣いや体験の劣化につながるため、
      <code>last_active</code> を定期的に監視し、非アクティブなユーザーを検出する必要がありました。
    </p>
    <p><strong>工夫：</strong><br>
      <code>last_active</code> の更新時刻を確認し、一定時間無操作のクライアントをルームから自動退出させる機能を実装しました。<br>
      またホストがタイムアウトした場合は、全クライアントにルーム終了を通知し、ルーム情報ごと削除されるようにしました。
    </p>
    <p><strong>効果：</strong><br>
      長時間無操作のユーザーによるリソース占有を防ぎ、チャットの安定運用と適切なルーム管理を実現しました。
    </p>
  </li>
</ul>
